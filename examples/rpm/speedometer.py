...
# Initialize serial
if (not test): 
 ser = None # serial.Serial('/dev/ttyUSB0',9600)
...
display_size=width, height=480,320 # Size of the Adafruit screen
...
display_font = pygame.font.Font("monaco.ttf", 120)

rpm_font = pygame.font.Font("monaco.ttf", 40)
 ...
 while (True):
  ...
  if not connected: 
     try: 
        ...
        print ("Could not obd yo" ) # str(ex))
        
  else:
     response = connection.query(obd.commands.RPM, force=True) # send the command, and parse the response
     print(response.value) # returns unit-bearing values thanks to Pint 
  
     if response.value != None:
        r = str(response.value).split ( '.' )
        rpm = int(r[0])
